...
public class LineUpToTag extends Command {
  ...
  private int lastTarget;
    ...
    @Override
    public void execute(){
        // If you see a tag that is for current alliance then determine where robot is
        // and position desired in front of tag
        // if (RobotContainer.getInstance().m_vision.AllianceTargetAquired()) {
        //     int fidID = (int) LimelightHelpers.getFiducialID(_limelightName);
        //     if ((fidID >= 0) && (fidID <= 15)) {
        //       //  lastTarget = fidID;
        //         var robotPose2d =  LimelightHelpers.getBotPose2d_wpiBlue(_limelightName); //m_poseProvider.get();
        //         // var robotPose = 
        //         //     new Pose3d(
        //         //         robotPose2d.getX(),
        //         //         robotPose2d.getY(),
        ...
        //         // take the current robot pose and add PI radians to say the camera is on the back of the robot.
        //         Pose2d newPose  = new Pose2d(
        //             robotPose2d.getX(),
        //             robotPose2d.getY(),
        //             new Rotation2d(robotPose2d.getRotation().getRadians() + Math.PI)
        ...
        //         // offset from cameral to middle of robot
        //         newPose = newPose.transformBy(new Transform2d(new Translation2d(Units.inchesToMeters(10.5),0),new Rotation2d()));
        //         SmartDashboard.putString("bot pose", newPose.toString());
        //         // Lookup Apriltag pose and transform it to position to drive to (line up position)
        //         Pose2d goalPose = TagApproaches.getInstance().DesiredRobotPos(lastTarget);
        //          SmartDashboard.putString("goal pose", goalPose.toString());
        //     } // else { lastTarget = 0; }
        // } //else { lastTarget = 0; }
        ...
        if (lastTarget == 0 ) {
            ...
            // dah - I replaced the two lines below with the line above
            //Pose3d targetPose = FieldLayout.getTagPose(fidID).get();
